class Player {
  ...
  resumeGame() {
    ...
    } else if (this.resumeCount === 3) {
      this.upgradeSpaceship2();
    } else if (this.resumeCount === 4) {
      this.upgradeSpaceship3();
    }
  }
  ...
  shoot() {
    ...
    if (this.resumeCount === 2) {
      ...
      numBullets = 2; // Adjust this value based on your requirements
    } else if (this.resumeCount === 3) {
      console.log(this.score);
    ...
    for (let i = 0; i < numBullets; i++) {
      this.bullets.push(
        new PlayerBullet(
          this.x - this.r + bulletOffset * (2 * i + 1),
          this.y,
          this.playerIsUp()
        )
      );
  ...
  draw() {
    ...
    if (this.score == 50 && !this.nftShown["1"]) {
      this.gamePaused = true;
      this.pauseGame("1");
      this.level = 2; // Update the level to 2
    } else if (this.score == 100 && !this.nftShown["2"]) {
      this.gamePaused = true;
      this.pauseGame("2");
      this.level = 3; // Update the level to 3
    } else if (this.score == 200 && !this.nftShown["3"]) {
      this.gamePaused = true;
      this.pauseGame("3");
      this.maxBullets = 8;
      this.level = 4; // Update the level to 4
    } else if (this.score == 300 && !this.nftShown["4"]) {
      this.gamePaused = true;
      this.pauseGame("4");
      this.lives += 5; // Grant extra lives in level 4
    }
  }
